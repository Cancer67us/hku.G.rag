...
def report_score(gold_path, predict_path):
    ...
    for gold, pred in zip(gold_info, pred_info):
        ...
        pred = pred["answer_original"].strip()
        if gold == "no answer" and pred != gold:
            score = 0.0
        elif gold == "no answer" and pred == gold:
            score = 1.0
        else:
            ...
            # jaccard计算的是预测中的关键词里面, 占了总关键词的多少.
            score = 0.1 * keyword_score + 0.9 * semantic_score
            # 关键词预测和语义预测各占一半分数.
        gold_info[idx]["score"] = score
        gold_info[idx]["predict"] = pred
        idx += 1
        print(f"Prediction: {question}, Score: {score}")
...
if __name__ == "__main__":
    ...
    # Path to the standard answer
    gold_path = "./data/7215_gold.json"
    print("Read gold from %s" % gold_path)

    # Path to the prediction file
    predict_path = "./data/7215_result.json"
    ...
    print("\n")
    print("=" * 100)
    print(f"Number of predicted questions: {len(results)}, Final prediction score: {final_score}")
    print("=" * 100)

    # Path to the results file
    metric_path = "./data/7215_metrics.json"
    results_info = json.dumps(results, ensure_ascii=False, indent=2)
    with open(metric_path, "w") as fd:
        fd.write(results_info)
    print(f"\nResults file saved to {metric_path}")
